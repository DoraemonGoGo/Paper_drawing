...
# 读取CSV文件
file_path = 'E:/Desktop/paper/Study2_Data/rection.xlsx'  # 请替换为实际文件名
...
heavy_light_recall = df[df['显示方式'].isin(['Graded', 'Heap-up', 'Icon'])]  # 只选择Heavy和Light负荷
...
# 调整Y轴范围，使最低点为 0.5
ax.set_ylim(0.5, 0.7)  # 使最大值自动适应数据
ax.set_xlim(-0.3, 2.2)
# 调整 x 轴刻度的字体大小
...
ax.spines['left'].set_bounds(0.5, 0.7)

# 强制让底部的 X 轴只显示三个点之间的范围
ax.spines['bottom'].set_bounds(0, 2)  # 限制X轴的显示范围从0到2，和X轴刻度相同

# 强制将 X 轴设置在 Y 轴的 0.3 位置
ax.spines['bottom'].set_position(('data', 0.4875))  # 让X轴位于Y轴的0.3位置

# 添加标题和标签
plt.ylabel('反应时间(s)', fontsize=16, fontweight='bold')
# ax.text(0.4, -0.15, '(a)', transform=ax.transAxes, ha='center', va='top', fontsize=18, fontweight='normal', fontname='Times New Roman')
ax.text(0.45, -0.2, '(a) 反应时间', transform=ax.transAxes, ha='center', va='top', fontsize=17, fontweight='normal')
...
# 设置图例，去掉框线
# 使用 bbox_to_anchor 将图例稍微向右推一点，以视觉上对齐错误率图
ax.legend(loc='upper right', bbox_to_anchor=(1.04, 1), frameon=False, fontsize=16)

# 保存为矢量图
plt.savefig("E:/Desktop/paper/Study2_Data/rection_zh.jpg", format="jpg", bbox_inches='tight', dpi=300)  # 保存为 JPG 格式
