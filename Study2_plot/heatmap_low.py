...
def plot_heatmaps(folder):
    ...
    for i, file_name in enumerate(file_names):
        ...
        x_less_than_minus_0_5 = (x < -6.5).sum() / len(x)  # x < -0.5 的比例
        ...
        # ax.axvline(x=-4, color='#2DA2FE', linestyle='--', label='x = -4')  # 垂直虚线
    ...
    # 设置整体图表的标题
    # plt.suptitle('Heatmaps of Eye Gaze Data(Low Task)', fontsize=18)
    # plt.tight_layout(rect=[0, 0, 1, 0.96])  # 调整布局
    fig.supxlabel('(a) 轻负荷', fontsize=26)

    # 保存为矢量图
    plt.savefig("E:/Desktop/paper/Study2_Data/merged_files/heatmap_low.png", format="png",
...
# 示例用法
folder = 'E:/Desktop/paper/Study2_Data/merged_files/LowTask'  # 替换为实际的文件夹路径
