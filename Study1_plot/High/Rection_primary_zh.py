...
std_df2 = df.groupby(['角度', '效果'])['主要任务反应时间'].std().unstack()  # 计算标准差
...
# 设置标题和Y轴标签
# ax.set_title('主要任务反应时间 (重负荷)', fontsize=16, fontweight='bold')
ax.set_ylabel('反应时间 (s)', fontsize=16, fontweight='bold')
# ax.set_xlabel('(a)', fontsize=16, fontweight='bold', fontname='Times New Roman')
ax.text(0.4, -0.2, '(a) 反应时间 (重负荷)', transform=ax.transAxes, ha='center', va='top', fontsize=17, fontweight='normal')
# ax.set_ylim(0, max(mean_df.max()) + 1)  # 调整Y轴范围

# 显示图例
ax.legend(loc='upper right', prop={'weight': 'bold', 'size': 16}, frameon=False)
# 保存为矢量图
