...
# 计算均值和标准误差
mean_df = df.groupby(['角度', '效果'])['主要任务反应时间'].mean().unstack()
std_df = df.groupby(['角度', '效果'])['主要任务反应时间'].apply(stats.sem).unstack()
...
# 设置标题和Y轴标签
# ax.set_title('主要任务反应时间 (轻负荷)', fontsize=16, fontweight='bold')
ax.set_ylabel('反应时间 (s)', fontsize=16, fontweight='bold')
ax.text(0.4, -0.2, '(c) 反应时间 (轻负荷)', transform=ax.transAxes, ha='center', va='top', fontsize=17, fontweight='normal')
# ax.set_ylim(0, max(mean_df.max()) + 1)  # 调整Y轴范围

# 显示图例
legend = ax.legend(loc='upper right', prop={'weight': 'bold', 'size': 16}, frameon=False)

# 保存为矢量图
plt.savefig("E:/Desktop/paper/Study1_Data/Low Task/" + file_name + "_zh.jpg", format="jpg", bbox_inches='tight', dpi=300)  # 保存为 PDF 矢量图格式
