...
file_name = "次要任务反应时间Low"
# 假设你的文件名是 "data.xlsx"，请替换为你的实际文件名和路径
file_path = "E:/Desktop/paper/Study1_Data/Low Task/" + file_name + ".xlsx"

# 读取Excel文件
df = pd.read_excel(file_path)

# 计算均值和标准误差
mean_df = df.groupby(['角度', '效果'])['次要任务反应时间'].mean().unstack()
std_df = df.groupby(['角度', '效果'])['次要任务反应时间'].apply(stats.sem).unstack()
...
ax.spines['bottom'].set_position(('data', 0.9))  # 让X轴位于Y轴的0.3位置
...
# 设置标题和Y轴标签
# ax.set_title('次要任务反应时间 (轻负荷)', fontsize=16, fontweight='bold')
