...
def calculate_ratio_for_file(filename):
    try:
        ...
        for line in lines[1:]:  # 假设第一行为标题行
            if len(line) >= 4:  # 确保至少有五列
                value = float(line[3])  # 提取第五列的值
                if value < 0:
...
# 获取文件夹中以low开头的csv文件
folder_path = 'E:/Desktop/Study1_Data/张博宣-8.6/Low Task'  # 替换为你的文件夹路径
csv_files = glob.glob(os.path.join(folder_path, 'low*.csv'))
